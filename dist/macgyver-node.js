...
$macgyver.register = function (id, options) {
  ...
  if (id && options) {
    // both id + options
    options.id = id;
  } else if (lodash.isPlainObject(id)) {
    // Just options
    var _ref = [id.id, id];
    id = _ref[0];
    options = _ref[1];
  ...
  if (!lodash.isString(options.id) || !options.id.startsWith('mg')) throw new Error('Widget IDs must be simple strings beginning with "mg*"');
  $macgyver.widgets[options.id] = _objectSpread2({
    title: lodash.startCase(options.id),
    userPlaceable: true,
    category: 'Misc',
    icon: 'far far-square',
    format: false,
...
$macgyver.forms.getPrototype = function (id) {
  return $macgyver.flatten(lodash.isString(id) ? $macgyver.$forms[id].config : id, {
    type: 'spec',
    want: 'array',
    wantDataPath: true
...
$macgyver.forms.run = function (id, action) {
  ...
  if (actionReadable && $macgyver.$forms[id].$props.actions && $macgyver.$forms[id].$props.actions[actionReadable]) {
    ...
    var func = lodash.isString($macgyver.$forms[id].$props.actions[actionReadable]) ? $macgyver.$forms[id][actionReadable] : $macgyver.$forms[id].$props.actions[actionReadable]; // Tidy up actionArgs

    actionArgs = lodash(actionArgs || '').trim('()').split(',').map(function (i) {
...
$macgyver.inject = function (component) {
  ...
  if (component.$props.config.$dataPath) {
    var refresher = function refresher() {
      component.data = lodash.get($macgyver.$forms[component.$props.form].$props.data, component.$props.config.$dataPath);
    };
...
$macgyver.flatten = function (root, options) {
  ...
  if (settings.filter && !lodash.isFunction(settings.filter) && lodash.isObject(settings.filter)) settings.filter = lodash.matches(settings.filter);
  if (settings.want != 'object' && settings.want != 'array') throw new Error('$macgyver.flatten({want}) can only be "object" or "array"');

  if (settings.type == 'auto') {
    if (root.items) {
      settings.type = 'spec';
    } else if (lodash.every(root, function (k, v) {
      return !v.items;
    })) {
      settings.type = 'data';
    } else {
  ...
  var depthScanner = function depthScanner(node, dataPath, specPath, depth) {
    if (!lodash.isObject(node)) return; // Add to bucket of found objects?

    if (!settings.filter // No filter
    || settings.filter.call(node, node, path, depth) // OR we pass the filter
    ) {
        if (settings.wantDataPath) node[lodash.isString(settings.wantDataPath) ? settings.wantDataPath : 'path'] = dataPath.concat([node.id]).filter(function (i) {
          return i;
        }).join('.');
        if (settings.wantSpecPath) node[lodash.isString(settings.wantSpecPath) ? settings.wantSpecPath : 'specPath'] = specPath.join('.');
    ...
    if (lodash.isArray(recursionSubject) && (!settings.filterChildren // No filter
...
$macgyver.neatenSpec = function (spec, options) {
  var settings = _objectSpread2({
    ...
    convertShorthandDetect: function convertShorthandDetect(spec) {
      return lodash.isPlainObject(spec) // Simple object
      && !lodash.has(spec, 'type') // It doesn't have a type key (i.e. there is only one item in this object
      && lodash.every(spec, function (v, k) {
        return !lodash.has(v, 'id') && (!lodash.has(v, 'type') || v.type != 'mgContainer');
      });
    },
    ...
    convertShorthandTranslate: function convertShorthandTranslate(spec) {
      return {
        type: 'mgContainer',
        items: lodash.map(spec, function (v, k) {
          return _objectSpread2({
            id: k
          }, v, {
            type: function () {
              if (lodash.isString(v)) v = {
                type: v
  ...
  var layout = settings.clone ? lodash.cloneDeep(spec) : spec; // Output spec

  if (settings.deblank && lodash.isEmpty(layout)) {
  ...
  if (settings.convertArray && lodash.isArray(layout)) {
  ...
  $macgyver.flatten(layout, {
    ...
    if (!widget.type || !$macgyver.widgets[widget.type]) {
      ...
      Object.assign(widget, lodash($macgyver.widgets[widget.type].config).pickBy(function (v, k) {
        return !lodash.has(widget, k) && lodash.has(v, 'default');
      }).mapValues(function (v) {
        return v["default"];
      }).value());
    }

    if (settings.widgetTitles && !widget.title && widget.id) widget.title = lodash.startCase(widget.id);
  });
  return layout;
};
/**
...
$macgyver.fetch = function (url, options) {
  ...
  .then(function (session) {
    if (!lodash.isEmpty(session.settings.mappings)) {
      // Extract mappings
  ...
  .then(function (session) {
    var checkRequired = lodash(session.settings).pickBy(function (v, k) {
      return k.required && !lodash.has(session.mappings, v);
    }).map(function (v, k) {
      return k;
    }).value();
  ...
  .then(function (session) {
    ...
    switch (session.settings.type) {
      case 'array':
        if (!lodash.isArray(session.response.data)) throw "Expected an array from data feed \"".concat(url, "\" but got a non-array");

        if (!lodash.isEmpty(session.mappings)) {
          var pickKeys = lodash.map(session.settings.mappings, function (v, k) {
            return session.mappings['$' + k];
          });

          session.output = session.response.data.map(function (doc) {
            return (// Remap data using mappings
              lodash.pick(doc, pickKeys)
            );
          });
        } else {
      ...
      case 'object':
        if (!lodash.isPlainObject(session.response.data)) throw "Expected object return from data feed \"".concat(url, "\" but got a non-plain-object");

        if (!lodash.isEmpty(session.mappings)) {
          session.output = lodash(session.mappings).mapKeys(function (v, k) {
            return k.replace(/^\$/, '');
          }) // Remove '$' prefix
  ...
  .then(function (session) {
    try {
      ...
      throw lodash.isString(session.settings.formatError) ? session.settings.formatError : session.settings.formatError(err);
    }
  }) // }}}
  ["catch"](function (err) {
...
$macgyver.utils.getPath = function (target, path, fallback) {
  return lodash.get(target, path, fallback);
};
...
//# sourceMappingURL=macgyver-node.js.map
